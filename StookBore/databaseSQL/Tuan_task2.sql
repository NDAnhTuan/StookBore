...
SET temp = (
	...
	INSERT INTO grand_total_view (order_id, grand_total)
    VALUES ( NEW.order_id,temp);
END //
delimiter ;
		...
		BEGIN
			...
			ELSE
				BEGIN
					...
					SET grant_temp = (
                    SELECT
						IF(grand_total*amount_grant > max_amount_grant,grand_total - max_amount_grant,grand_total*(1-amount_grant))
					FROM grand_total_view WHERE order_id = NEW.order_id);
